...
class Home extends Component {
  state = {total: [], displayData: [], active: 0, selected: categories[0]}
  ...
  onClickItem = sel => {
    ...
    switch (sel) {
      case categories[0]:
        this.setState({
          displayData: total[0].categoryDishes,
          selected: categories[0],
        })
        break
      case categories[1]:
        this.setState({
          displayData: total[1].categoryDishes,
          selected: categories[1],
        })
        break
      case categories[2]:
        this.setState({
          displayData: total[2].categoryDishes,
          selected: categories[2],
        })
        break
      case categories[3]:
        this.setState({
          displayData: total[3].categoryDishes,
          selected: categories[3],
        })
        break
      case categories[4]:
        this.setState({
          displayData: total[4].categoryDishes,
          selected: categories[4],
        })
        break
      case categories[5]:
        this.setState({
          displayData: total[5].categoryDishes,
  ...
  render() {
    const {displayData, active, selected} = this.state
